...
export class Artifacts extends Client {
   ...
   * ```
   */
  async getGeneratingProcess(
    siteId: string
  ): Promise<Api.Artifacts.Generator.ProgressResponse> {
  ...
  async getDownloadURL(
    siteId: string,
    artifactId: string
  ): Promise<Api.Artifacts.PreviewURL> {
  ...
  async enablePreview(siteId: string, artifactId: string): Promise<string> {
    ...
    const subdomain =
      this.endpoint === "https://api.getshifter.io" ? "preview" : "previewdev";
  ...
  async publish(siteId: string, artifactId: string): Promise<""> {
    const path = this.getArtifactPath(
      siteId,
      `${artifactId}/deploy/shifter_cdn`
    );
    await this.post(path);
    return "";
  }
  ...
  async generate(
    siteId: string
  ): Promise<Api.Artifacts.Generator.StartResponse> {
    const path = this.getArtifactPath(siteId);
